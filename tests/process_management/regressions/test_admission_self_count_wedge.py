...
- 24GB flux exclusive head: a preload was admitted and recorded a planned charge, then its target was
  reclaimed before the load materialised. A dead target's reservation never grows, so the charge decayed by
  neither materialisation nor omission; only excluding dead/ended processes from the in-flight set releases it.
- 8GB SD1.5 resident and idle: the model was already resident and idle on the target, so dispatching it moved
  nothing, yet the dispatch gate withheld it because the resident weights (already in the committed floor) were
  re-priced.
"""
...
class TestResidentIdleDispatchReleasesImmediately:
    ...
    async def test_resident_idle_dispatch_is_not_held_over_a_full_card(self) -> None:
        """The gate releases the dispatch though the 8GB card's committed floor sits above capacity."""
