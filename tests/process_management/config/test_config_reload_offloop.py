...
async def test_reload_runs_blocking_load_off_loop_and_applies_on_loop() -> None:
    ...
    process_manager._load_bridge_data_blocking = fake_load  # type: ignore[assignment,method-assign]
    process_manager._apply_reloaded_bridge_data = fake_apply  # type: ignore[assignment,method-assign]

    await process_manager._reload_bridge_data_off_loop()
...
async def test_reload_skips_apply_when_nothing_to_load() -> None:
    ...
    process_manager._load_bridge_data_blocking = lambda: None  # type: ignore[assignment,method-assign]
    process_manager._apply_reloaded_bridge_data = lambda bridge_data: applied.setdefault("called", True)  # type: ignore[assignment,method-assign]
...
async def test_schedule_config_reload_runs_the_reload() -> None:
    ...
    process_manager._reload_bridge_data_off_loop = fake_reload  # type: ignore[assignment,method-assign]

    process_manager._schedule_config_reload()
