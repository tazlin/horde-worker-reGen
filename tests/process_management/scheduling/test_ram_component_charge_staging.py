...
The RAM-verdict path (:func:`decide_ram_admission` behind :meth:`InferenceScheduler._apply_ram_verdict`)
prices a disaggregation-class preload at its UNet-only component charge (read torch-free from the checkpoint's
component-identity sidecar) instead of the whole checkpoint, credits the charge to zero when the checkpoint is
already staged on the target, falls back to the whole-checkpoint charge when no sidecar is available, and
reconciles the component charge against measured RSS growth. A job the whole-checkpoint charge would defer is
dispatched at the UNet-only price.
...
async def _charge(scheduler: InferenceScheduler, job: object) -> float | None:
    """The component charge the snapshot prices for ``job`` onto slot 0."""
    await track_popped_job_async(scheduler._job_tracker, job)
    return preload.component_charge_mb(scheduler.snapshot(), str(job.id_), 0)  # type: ignore[attr-defined]
...
class TestComponentChargeSelection:
    ...
    async def test_disaggregation_class_charges_unet_residual(self) -> None:
        ...
        assert await _charge(scheduler, job) == pytest.approx(_RESIDUAL_MB)

    async def test_monolithic_job_charges_whole(self) -> None:
        """A non-disaggregation-class job returns None so the whole-checkpoint charge stands."""
        scheduler = _make_inference_scheduler(process_map=ProcessMap({0: _idle_target()}))  # predicate off
        _pin_sidecar(scheduler, _FakeSidecar(int(_RESIDUAL_MB * _MB)))
        job = make_job_pop_response("mono_model")
        assert await _charge(scheduler, job) is None

    async def test_missing_sidecar_charges_whole(self) -> None:
...
class TestResidencyCredit:
    ...
    async def test_held_checkpoint_credits_charge_to_zero(self) -> None:
        ...
        job = make_job_pop_response("disagg_model")
        assert await _charge(scheduler, job) == pytest.approx(0.0)

    async def test_absent_checkpoint_charges_full_residual(self) -> None:
...
class TestDeclinedJobRepricesWhole:
    ...
    async def test_declined_job_charges_whole(self) -> None:
...
class TestMissingSidecarParity:
    ...
    async def test_no_sidecar_matches_prefeature_reuse_credit(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        async def _run(*, disaggregation_class: bool) -> tuple[bool, ReuseCreditRecord | None]:
        ...
        disagg_admitted, disagg_record = await _run(disaggregation_class=True)
...
class TestComponentAdmissionLiveness:
    ...
    async def test_unet_only_pricing_dispatches_what_whole_pricing_defers(
