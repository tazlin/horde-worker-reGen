...
class TestControlnetWorkflowDispatch:
    """A controlnet workflow job is priced, not serialised: no worker-wide hold stands in front of the pool.

    The extra weights a ``qr_code`` job loads are charged per card by the admission and overlap gates
    (see ``resource_budget._job_feature_kinds``), the same way a batch or a card-demanding model is. The
    dispatch cycle therefore treats such a job like any other: an idle pool dispatches it at once, and a
    second lane fills beside it whenever the card has the measured room.
    """
    ...
    async def _controlnet_head_scheduler(
        self,
        job_tracker: JobTracker,
        *,
        max_concurrent: int,
    ) -> tuple[InferenceScheduler, dict[str, object], ImageGenerateJobPopResponse, ImageGenerateJobPopResponse]:
        """Two idle lanes holding distinct SDXL models; the head is a qr_code job, the follower a plain one."""
        head_slot = make_mock_process_info(1, model_name=_SDXL_A, state=HordeProcessState.PRELOADED_MODEL)
        sibling_slot = make_mock_process_info(2, model_name=_SDXL_B, state=HordeProcessState.PRELOADED_MODEL)
    ...
    async def test_an_idle_pool_dispatches_a_controlnet_head_under_a_one_thread_cap(
        self,
        job_tracker: JobTracker,
    ) -> None:
        """With one thread the head runs alone; the cap, not a workflow hold, is what stops the second."""
        scheduler, reference, head, _follower = await self._controlnet_head_scheduler(job_tracker, max_concurrent=1)

        await scheduler.run_scheduling_cycle(reference)  # type: ignore[arg-type]

        assert head in job_tracker.jobs_in_progress
        assert len(job_tracker.jobs_in_progress) == 1

    async def test_a_second_lane_fills_beside_a_running_controlnet_job_once_it_has_headway(
        self,
        job_tracker: JobTracker,
    ) -> None:
        """A qr_code job sampling on one lane does not keep the other idle: the overlap gate decides, not a hold.

        The running job has made size-appropriate headway and the card has measured room, the same two
        conditions under which any heavy job admits a second lane; a controlnet workflow is no exception.
        """
        scheduler, reference, head, follower = await self._controlnet_head_scheduler(job_tracker, max_concurrent=2)
        await mark_job_in_progress_async(job_tracker, head)
        running = scheduler._process_map[1]
        running.last_process_state = HordeProcessState.INFERENCE_STARTING
        # Re-recorded while the slot is sampling, so the reference is execution ownership, not preload intent.
        running.last_job_referenced = head
        running.batch_amount = 1
        running.last_total_steps = 20
        running.last_current_step = 18
        running.last_heartbeat_percent_complete = 90

        await scheduler.run_scheduling_cycle(reference)  # type: ignore[arg-type]

        assert follower in job_tracker.jobs_in_progress
        assert len(job_tracker.jobs_in_progress) == 2
