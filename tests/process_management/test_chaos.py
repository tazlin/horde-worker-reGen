...
*intended* resilient behaviour. Where current code does not yet provide it, the test is marked
``xfail`` with the roadmap phase that will: the xfail flipping to a pass is the acceptance signal
for that phase. See ``tests/e2e/test_chaos_e2e.py`` for the full spawned-process counterparts.
"""
...
@pytest.mark.xfail(
    reason="Phase 2: INFERENCE_STARTING has no timeout and is_stuck_on_inference ignores a "
    "process that never emitted a step heartbeat (hung at 0%).",
    strict=False,
)
def test_hung_before_first_step_is_detected() -> None:
    ...
    fire; only a per-state / heartbeat-aware check can catch the wedge. Today neither does.
    """
...
@pytest.mark.xfail(
    reason="Phase 2: an idle slot stuck in WAITING_FOR_JOB while jobs are pending has no timeout.",
    strict=False,
)
...
@pytest.mark.xfail(
    reason="Phase 2: replacing a crashed process only releases the inference semaphore when it died "
    "in INFERENCE_STARTING; a crash in another holding state orphans the semaphore.",
    strict=False,
)
...
@pytest.mark.xfail(
    reason="Phase 2: there is no crash-loop circuit breaker; a permanently-failing slot is respawned "
    "without bound instead of being quarantined.",
    strict=False,
)
