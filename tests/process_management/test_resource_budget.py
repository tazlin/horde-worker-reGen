...
from horde_worker_regen.process_management.resource_budget import BudgetVerdict, VramBudget
...
class TestVramBudget:
    ...
    def test_cold_start_admits_when_no_telemetry(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert verdict.free_vram_mb is None
    ...
    def test_admits_when_estimate_unavailable(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert verdict.fits is True
        assert verdict.predicted_vram_mb is None
    ...
    def test_verdict_reason_strings(self) -> None:
        ...
        assert "cold start" in BudgetVerdict(True, None, None, 2048.0).reason()
        assert "no burden estimate" in BudgetVerdict(True, None, 1000.0, 2048.0).reason()
        assert "does NOT fit" in BudgetVerdict(False, 4000.0, 1000.0, 2048.0).reason()
        assert "fits" in BudgetVerdict(True, 1000.0, 8000.0, 2048.0).reason()
...
class TestPreloadBudgetGate:
    ...
    async def test_preload_proceeds_when_within_budget(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """With ample free VRAM the budget admits and the preload is sent."""
        monkeypatch.setattr(resource_budget, "predict_job_vram_mb", lambda job, baseline: 4000.0)
