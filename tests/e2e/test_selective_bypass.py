...
def test_skip_inference_only() -> None:
    ...
    process_manager = make_testable_process_manager(bridge_data=bd)
...
def test_bridge_data_dry_run_fields_default_false() -> None:
    ...
    bd = make_mock_bridge_data()
    assert bd.dry_run_skip_inference is False
    assert bd.dry_run_skip_safety is False
    assert bd.dry_run_skip_api is False
    assert bd.dry_run_inference_delay == 1.0
