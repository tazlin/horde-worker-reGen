"""The TUI's F2 (local pause/resume) action uses supervisor_paused, not the aggregate maintenance_mode.
...
clears supervisor_paused. Reading the aggregate here would make every F2 press send RESUME forever,
...
def test_f2_sends_pause_when_not_locally_paused_even_with_server_maintenance_active() -> None:
    """F2 pauses locally even when aggregate maintenance_mode is True due to server-side state.

    The horde may force maintenance while supervisor_paused is False: the aggregate is True but F2
...
def test_f2_sends_resume_when_locally_paused() -> None:
    """F2 resumes when the operator has locally paused the worker."""
...
def test_f2_sends_pause_when_snapshot_is_none() -> None:
    """F2 falls through to PAUSE when there is no snapshot (worker not yet started)."""
    stub = Mock()
