...
def format_progress_event(event: BenchmarkProgressEvent) -> str | None:
    ...
    if isinstance(event, LevelProgress):
        return f"     . {event.level_id}: {_progress_detail(event)}"
...
def _progress_detail(event: LevelProgress) -> str:
    """Render the metric fragment of a :class:`LevelProgress` event."""
    if event.jobs_expected is not None:
        parts = [f"{event.jobs_completed}/{event.jobs_expected} jobs"]
    else:
        parts = [f"{event.jobs_completed} jobs"]
