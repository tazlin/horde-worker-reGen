"""A best-effort 'is there a newer release?' check for the dashboard.

Compares the running version against the latest GitHub release and, when newer, lets the app surface a
...
library (no extra dependency, no heavy import in the TUI parent), and degrades to "no update" on any
error so an offline launch is silent.
"""
...
__all__ = ["RELEASES_URL", "UpdateInfo", "check_for_update", "current_version"]
