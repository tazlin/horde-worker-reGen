...
* `closed_loop` marks the simulator rows under `tests/process_management/liveness/` (incident scenarios, golden traces, the bounded dispatch matrix, the chaos core slice). They stay in the default sweep and run after the rest of `process_management`, but each costs seconds, so while iterating on a scheduling change run `-m "not closed_loop"` plus the rows the change names, and run the whole band once before committing.
    ...
    * Two caveats. The fastest-first run order in `tests/conftest.py` is a serial property: `-n` hands rows to workers as they come free, so parallelise the band on its own rather than a mixed selection that relies on the ordering to surface a cheap break first. And one pytest invocation per working copy still holds, since the workers live inside a single invocation; two `-n` runs in the same checkout abort each other exactly as two serial ones do.
