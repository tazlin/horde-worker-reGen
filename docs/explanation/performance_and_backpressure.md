...
to stop all image intake. When the worker already has two or more accepted
...
## Serviceability: what a card can host, and the constrained lane

Before any of the narrowings above, each offered model is judged against the card it would run on
(`resources/model_serviceability.py`). A model whose smallest legal job cannot fit `total - baseline -
noise` is never advertised. A model whose smallest job fits but whose `max_power` job does not is
...
2. **No single-inference hold**: nothing worker-wide serializes launches. A
   controlnet workflow's extra weights, a batch's multiplied activation peak, and
   a card-demanding model are each priced per-card by the admission and
   concurrent-overlap gates below, so a second lane runs whenever the card has
   the measured room for it and waits only when it does not.
