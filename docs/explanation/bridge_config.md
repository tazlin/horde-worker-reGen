...
| `text_backend_managed`           | `true`                    | The worker launches and supervises the backend itself: obtains the executable, starts it on `text_backend_port` with `text_model_path`, relaunches it if it exits, stops it on shutdown. `false` attaches to a backend you run at `kai_url`. |
| `text_model_path`                | unset                     | The model file the managed backend loads (a GGUF for koboldcpp). Required when `scribe` and `text_backend_managed` are both true; the config is refused without it.                                                              |
