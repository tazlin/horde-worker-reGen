...
| `alchemy_caption_enabled`   | `false` | Opts into caption forms. Off by default because captioning loads BLIP into the safety process; BLIP remains in RAM but is returned to CPU between forms so it is not fixed GPU residency. |
...
| `text_model`                     | unset                     | Which model a worker-run backend loads: a catalogued model's name, as the text model reference spells it, or the path to a model file of your own. An existing path is that file; anything else is looked up as a name. Required when `scribe` and `text_backend_managed` are both true; the config is refused without it. |
| `text_models_dir`                | unset                     | Where model files the worker fetches are kept. Unset means `<AIWORKER_CACHE_HOME>/text_models`, which is the worker's own choice because the model reference publishes no folder for text models. A `text_model` naming a path is read where it sits. |
| `text_model_name`                | unset                     | Override the advertised model name, as the text model reference spells it (`author/Model`, plus any quant suffix). Unset follows the resolved model: the record's name, the file's stem, or what an attached backend reports. Give the plain name: the backend prefix is derived, and a prefixed name is rejected. |
...
| `text_stall_seconds`             | `30`                      | Seconds with no text arriving before a generation is abandoned and the job faulted. The answer streams, so silence tells a wedged backend from a slow one, which the deadline above cannot. The first token after the backend starts is exempt: a cold backend takes seconds to produce anything. |
| `text_backend_kind`              | `koboldcpp`               | Which backend program serves text, in the horde's own naming. Decides how it is launched, which driver talks to it and how the advertised name is prefixed. koboldcpp is the first supported backend; an unsupported one is refused by name at start-up. |
| `text_backend_managed`           | `true`                    | The worker launches and supervises the backend itself: obtains the executable and the model file, starts it on `text_backend_port`, relaunches it if it exits, stops it on shutdown. `false` attaches to a backend you run at `kai_url`. |
| `text_backend_executable`        | unset                     | A backend program to launch instead of the one the worker provisions (your own build, a newer release, a backend the worker cannot yet download). A `.py` path is a source checkout's entry script, run under the worker's own interpreter.                                                                                |
| `text_backend_password`          | unset                     | The password a backend you run yourself (`text_backend_managed: false`) requires, sent with every request. A refused credential is reported as such and stops the flow asking for jobs, because waiting cannot fix it; a corrected value is picked up on reload. |
| `text_backend_accelerator`       | `auto`                    | Which compute path the managed backend runs on: `auto`, `cuda`, `vulkan` or `cpu`. `auto` follows the backend this install was set up for (`HORDE_WORKER_BACKEND`, then `bin/backend`): NVIDIA runs text on CUDA, AMD on Vulkan, a CPU build on the CPU, and an install that recorded nothing is detected. The same answer picks which koboldcpp build is downloaded. See [Run a text worker](../how-to/run-a-text-worker.md). |
| `text_backend_port`              | `5001`                    | Loopback port the managed backend listens on. A port something else still holds defers the launch rather than guessing another.                                                                                                 |
| `text_gpu_layers`                | `99`                      | How much of the model the managed backend puts on the card, in the backend's own unit (layers for llama.cpp-based backends). Large means all of it; `0` keeps it on the CPU.                                                    |
| `text_gpu_device_index`          | unset                     | Stable device index of the card the managed backend uses. Unset shares the lowest driven card. On a multi-GPU host, naming a card removes it from the image device map so text owns it; a single card stays shared.               |
