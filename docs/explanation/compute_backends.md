...
1. the resolved feature set is non-empty (the same `HORDE_WORKER_FEATURES` resolution used for the
   ...
   out), **and**
2. the checked-in utilities lock (`requirements/utilities/uv.lock`) is present.

It syncs the venv from that lock against the same pinned managed CPython, then records a stamp (the backend
token plus the utilities-lock SHA256) so a subsequent launch re-provisions only when the lock or backend
actually changes. A provisioning failure is non-fatal: the worker still runs, the failure is warned, and the
next sync retries it.

**The lockfile mechanism.** `requirements/utilities/` is a self-contained uv project (a `pyproject.toml`
plus a committed `uv.lock`, one lock with a build extra per locked torch build: `cu126`, `cu130`, `cu132`,
`cpu`). The bootstrap provisions with `uv sync --locked --extra <backend_token>` (pointing
`UV_PROJECT_ENVIRONMENT` at the peered utilities venv), so it installs strictly from the lock's recorded
sources: deterministic across machines, reusing the main `.venv`'s cached torch/CUDA wheels, and immune to a
box's ambient extra-index. The lock pins `torch`/`torchvision` to the same build the root `uv.lock` resolves
(a guard test enforces this) so the shared cache actually hits. A backend not covered by the lock has nothing
to provision (the step stays a no-op). See `requirements/README.md` for the layout and regeneration details.
