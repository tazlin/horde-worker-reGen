...
2. **Update**, matching how you installed:
   ...
   |----------------|-------------|
   | One-line installer | Run `update.cmd` (Windows) or `update.sh` (Linux/macOS) in your worker folder, or re-run the same `irm ... \| iex` / `curl ... \| sh` command. Both download the latest release and update in place, leaving your peered `<worker>-data` folder (models, cache, Python) untouched. |
   | winget | `winget upgrade Haidra.HordeWorker` |
   | Git clone | `git pull`, then `update-runtime.cmd` (or the `.sh` / `-rocm` variant) |
...
A managed install (one-line installer, `.exe`, or winget) previews what a dependency sync would
