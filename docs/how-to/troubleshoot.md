...
| Logs say `Not offering <model>: its smallest legal job (512x512) does not fit any card that serves it`, or the dashboard's Health tab shows a **Models** row "Not offered, cannot fit this card" | The card's VRAM minus its shared baseline (desktop, other apps) and the admission noise buffer is smaller than the model's resident weights plus a 512x512 sampling activation, so the worker never advertises it: a popped job for it would only fault. Remove the model, or free VRAM on the card (close other GPU apps, `safety_on_gpu: false`). |
| Logs say `Offering <model> at reduced max_power N (configured M)`, or the Health tab shows a **Models** row "Offered at reduced size, pops capped at N" | The model's smallest job fits the card but a job at your configured `max_power` would not, so the model is advertised on a pop of its own that asks the horde for jobs no larger than `max_power` N, one pop in every four; the other pops carry your other models at the configured `max_power`. This is expected on an 8 GB card serving SDXL beside a busy desktop; lower `max_power` to N to silence it, free VRAM on the card, or remove the model. A model whose core weights can never be resident on the card (Qwen-Image on 24 GB) is not capped this way: it streams from RAM with the whole card, and is offered whenever the card meets the baseline's recommended minimum. |
...
| One inference slot faults every job for minutes with "Too many open files" (`[Errno 24]` / EMFILE), then recovers (Linux) | The slot leaked file descriptors until it hit its per-process `RLIMIT_NOFILE` ceiling; from there **every** `open()` is refused, so its free-RAM probe (`/proc/meminfo`), its checkpoint, and its LoRA `.safetensors` all fail and it faults each job. It keeps heart-beating, so the hang watchdog cannot see it: only the "failed to load model" recovery eventually replaces the slot, after a long window of dropped jobs. The worker now hardens against this on two fronts: it **raises the soft descriptor limit to the hard limit at startup** (the PyTorch-recommended mitigation for tensor-sharing descriptor use, inherited by every child), and it **warns before exhaustion** (`... is nearing its file-descriptor ceiling`) so a leak is visible while the slot is still serving. This is a **descriptor leak, not memory pressure**: lowering `max_threads`/`queue_size` or the VRAM budget will not help. If it still recurs, the ceiling only delays a true leak: capture a support bundle (the per-process descriptor figures localize it) and run `horde-log diagnose` (it names the slot and the model that was running when the ceiling was hit). POSIX-only: Windows has no `RLIMIT_NOFILE` and a far higher handle ceiling, so this does not occur there. |
...
co-resident with near-zero free VRAM, rather than one model too large), file-descriptor exhaustion (a
slot that faults every job with "Too many open files" until it is replaced, which reads as an OOM but
needs a different fix), the horde forcing the worker into maintenance for dropping too many jobs (and ties it
