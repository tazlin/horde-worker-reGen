...
| One inference slot faults every job for minutes with "Too many open files" (`[Errno 24]` / EMFILE), then recovers (Linux) | The slot leaked file descriptors until it hit its per-process `RLIMIT_NOFILE` ceiling; from there **every** `open()` is refused, so its free-RAM probe (`/proc/meminfo`), its checkpoint, and its LoRA `.safetensors` all fail and it faults each job. It keeps heart-beating, so the hang watchdog cannot see it: only the "failed to load model" recovery eventually replaces the slot, after a long window of dropped jobs. This is a **descriptor leak, not memory pressure**: lowering `max_threads`/`queue_size` or the VRAM budget will not help. As a stopgap raise the soft descriptor limit (`ulimit -n`, or `LimitNOFILE=` in the systemd unit) so a slow leak takes far longer to reach the ceiling, and run `horde-log diagnose` (it names the slot and the model that was running when the ceiling was hit). POSIX-only: Windows has no `RLIMIT_NOFILE` and a far higher handle ceiling, so this does not occur there. |
