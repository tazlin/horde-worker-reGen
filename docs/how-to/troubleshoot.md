...
| Logs say `Offering <model> at reduced max_power N (configured M)`, or the Health tab shows a **Models** row "Offered at reduced size, pops capped at N" | The model's smallest job fits the card but a job at your configured `max_power` would not, so every pop that includes the model asks the horde for jobs no larger than `max_power` N. Other models in the same pop share that cap. This is expected on an 8 GB card serving SDXL beside a busy desktop; lower `max_power` to N to silence it, free VRAM on the card, or remove the model. |
