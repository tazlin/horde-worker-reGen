...
| Tab | What it shows |
|-----|---------------|
| **Overview** | Headline metrics (jobs submitted/faulted, queue depth, GPU duty cycle, kudos/hr), a system-RAM line (machine in-use/total plus the worker's own footprint broken down by inference/safety/orchestrator/download), worker identity, and a per-process summary colour-coded by **temperature** (hot / next / warm / priming / cold) so a primed slot reads differently from a genuinely empty one. |
