...
| **Live** | A compact **Scheduling** strip, a **Worker RAM by role** panel (the worker's resident RAM broken down by role, e.g. inference/safety, with the overall system figure in its title), then one panel per inference process. The strip shows RAM-governor status and the latest preload-admission decision; details density adds the active pop hold/pause, reclaim target, and gate reason. Process panels show state (and its temperature phrase, e.g. *sampling*, *primed*, *loading*), current model and job, a sampling progress bar, iterations/second, VRAM/RAM (current and peak), and heartbeat freshness. |
