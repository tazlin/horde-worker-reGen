...
| **Config** | A form over `bridgeData.yaml`, grouped by operator workflow with inline help, enforced bounds, masked secrets, explicit option controls for mode fields, and checkboxes for alchemy forms (including vectorize, palette, describe, and aesthetic). Comments and untouched keys are preserved. The editor keeps an always-visible summary of what the form serves, names unsaved changes and restart-only edits, runs live interlock checks before the worker starts, surfaces blocking conflicts on the relevant tab, and offers built-in hardware presets whose grouped setting changes can be unchecked before applying. Includes a dedicated models editor and a searchable model picker: search across name/description/tags/triggers, filter by baseline/SFW/NSFW/inpainting and by on-disk status, sort by any column, mark models to add to or remove from the load/skip lists, see which list each model is already in, and inspect a model's full record. A dedicated **Model pool** sub-tab configures the fixed model pool: the Max throughput mode one-switch preset plus every scalar pool knob (enable, seats, ranker, rotation and dwell windows, rescue, demand poll interval, and auto-download budget), with help written in the same terms as the Insights and Overview pool panels. The list of pinned models stays in `bridgeData.yaml` directly (it is a structured `{name, affinity}` list). Clear actions ask for confirmation before dropping staged work. |
| **Insights** | Live, actionable recommendations (low GPU duty cycle, VRAM pressure, fault rate, idle time, configuration mismatches, and model-pool guidance) and a recent-activity rollup. A **Model pool** panel is always shown: with the pool off it reads `Model pool: off` and notes the throughput-versus-variety trade; with the pool on it lists each seat (model, source, state, dwell, last-fulfilled age, empty pops, rescue countdown), the current advertising lane and demand-snapshot age, and the benched models. The recommendations are pool-aware in both directions: with the pool off and many distinct models served recently, it offers (as a preference, not a push) enabling the pool for more kudos/hr; with the pool on it flags a stale demand reading, seats that keep taking empty pops or have served nothing since seating, a seat stuck because the auto-download budget is spent, and notes when the pool is earning swap-free. |
...
reflex. Left off (the default), the worker serves your whole model list evenly and earns the widest variety of
jobs, but pays a model swap whenever a popped job needs a model that is not currently loaded. Turned on, it
commits to a small set of ready "seats" and shapes its pops so the horde returns work it can run swap-free:
more kudos per hour on a committed set, at the cost of serving fewer distinct models.

Enable it when the worker either sits idle waiting for rare-model demand or churns swapping between many models
(the Insights tab nudges you toward it when recent work spanned many models with the pool off). Reach for **Max
throughput mode** for the one-switch version (ranker on, a 50 GB auto-download budget), or set the individual
knobs for tighter control: `seats` at or below your inference-process count keeps the swap-free promise, the
demand `ranker` fills the seats you do not pin, and the rotation and dwell windows decide how fast the seat set
re-contests. Watch the result on the Insights **Model pool** panel: seats with a fresh fulfilled age are
earning their place, while rising empty pops mean the horde has little demand for that model on this card. Pins
themselves live in `bridgeData.yaml` as a `{name, affinity}` list, where affinity is a bias toward a seat, not
a lock.
