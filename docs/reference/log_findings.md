...
| `alchemy_pop_liveness` | varies | Alchemy work stopped reaching the horde while the worker held it back. The condition named in the evidence says what held it. | On a worker that serves alchemy only this is the whole intake path, so the silence means nothing was served at all. Fix the condition named, and report it with the log line. |
