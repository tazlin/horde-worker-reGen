...
| `scheduler_starvation_wedge` | varies | The VRAM arbiter's head-of-queue starvation diagnostic fired for a job on an idle, ample-VRAM device; critical if it escalated to a soft reset and faulted jobs, warning if the wedge cleared on its own. | An over-conservative per-process overhead or an unsatisfiable budget for that model; reduce process churn or relax the budget. |
