...
| Dashboard state channel                | `process_management/ipc/supervisor_channel.py` (`WorkerStateSnapshot`, `ProcessSnapshot`, `SupervisedProcessSnapshotSource` for processes outside `ProcessMap`, and `WorkerConfigSummary` with `enabled_worker_names`/`worker_display_name`, the one derivation of the worker's display identity) |
