...
| Obtaining, launching and keeping alive the external text backend | `process_management/lifecycle/text_backend_supervisor.py` (`TextBackendSupervisor`, which owns the backend's whole life from `TextBackendState.PROVISIONING` onwards and is also its dashboard row via `to_process_snapshot`); it is handed a `LaunchSpecFactory` that renders the command line through `text_backends/launch.py` (`build_launch_spec`, keyed by `TEXT_BACKENDS`) over an executable from `text_backends/provision.py`, which also fetches the model file (`ensure_text_model_file`) |
...
| Dashboard state channel                | `process_management/ipc/supervisor_channel.py` (`WorkerStateSnapshot`, `ProcessSnapshot`, `SupervisedProcessSnapshotSource` for processes outside `ProcessMap`, `WorkerConfigSummary` with `enabled_worker_names`/`worker_display_name`, the one derivation of the worker's display identity, and `WorkerEvent`/`WorkerEventKind`/`WorkerEventSink`, the transition ring the dashboards read) |
| The worker's own transitions, as a bounded ring | `process_management/resources/run_metrics.py` (`WorkerRunMetrics.record_event`, `recent_events`), which owns the ring so the finished-job events and the finished-job records are written by one recorder; every site with no metrics reference is handed `record_event` as a `WorkerEventSink` |
...
| `text_backends/`              | The worker's whole conversation with a text-inference program it does not own (koboldcpp, sonar): the six-verb `TextBackend` protocol and its three exception types (`protocol.py`), the KoboldAI HTTP driver both backends accept (`kobold_api.py`), and the in-process `FakeTextBackend` dry runs and tests drive instead of a binary. `model_catalogue.py` contributes the worker's measured text models to `horde_model_reference` as a `StaticModelProvider` (`register_text_model_catalogue`, source id `regen_text`) and resolves the one `text_model` key to a file, a name and a record (`resolve_text_model`) |
